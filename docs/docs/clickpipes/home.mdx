...
![ClickPipes stack](/images/clickpipes/clickpipes_stack.png)
...
| Azure Event Hubs | Streaming | Stable | Configure ClickPipes and start ingesting streaming data from Azure Event Hubs into ClickHouse Cloud. See the [Azure Event Hubs FAQ](/integrations/clickpipes/kafka/faq/#azure-eventhubs) for guidance. |
...
| Azure Blob Storage | Object Storage | Stable | Configure ClickPipes to ingest large volumes of data from object storage. |
...
| [MySQL](/integrations/clickpipes/mysql) | DBMS | Public Beta | Configure ClickPipes and start ingesting data from MySQL into ClickHouse Cloud. |
...
Steps:
1. Create a custom role `CREATE ROLE my_clickpipes_role SETTINGS ...`. See [CREATE ROLE](/sql-reference/statements/create/role) syntax for details.
2. Add the custom role to ClickPipes user on step `Details and Settings` during the ClickPipes creation.

![Assign a custom role](/images/clickpipes/cp_custom_role.png)
...
![Advanced settings for ClickPipes](/images/clickpipes/cp_advanced_settings.png)
